...
4. `docs/funding_data.js` — data for the public "AI lab equity funding, 2022–2026" section
   (`#funding`, added 2026-08-11): cumulative primary equity for OpenAI, Anthropic, xAI,
   Moonshot, DeepSeek, MiniMax, Z.ai. Generated by
   `~/research/lab-equity-funding/build_site_data.py` from that project's sourced
   `data/rounds.csv`; source-of-truth spreadsheet linked in the section footnote
...
9. `docs/valuation_data.js` — data for the public "AI lab valuations, 2022–2026" section
   (`#valuation`, added 2026-09-10): dated company valuations for the same seven labs as the
   funding chart, on a log axis, plus month-end market capitalisation for MiniMax and Z.ai
   after their January 2026 Hong Kong listings. Generated by
   `~/research/ai-lab-valuations/build_valuation_site_data.py` from that project's sourced
   `data/valuations_*.csv` and `data/market_caps.csv`; source-of-truth spreadsheet linked in the
   section footnote (Google Sheet ID `1NGuDTQiuuG38fFyBbbNZ2WZZ1MnZZ0PexBE1EG3Wbvs`). To update: edit
   the CSVs, rerun the build script, commit `docs/valuation_data.js`. The chart code sits in
   `docs/index.html` between `<!-- valuation -->` delimiters and is re-inserted by
   `~/research/ai-lab-valuations/insert_valuation.py`. While the build still reads any
   `*_provisional.csv`, the payload carries `provisional: true` and the page prints a
   "Provisional data" line under the chart.
